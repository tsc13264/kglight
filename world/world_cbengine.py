...
class Intersection(object):
    ...
    def _get_lane_link(self):
        lane_link = []
        for idx, road in enumerate(self.in_roads):
            for lane, direction_lst in road["lanes"].items():
                if direction_lst == [1, 0, 0]:
                    lane_link.append((
                        lane, list(self.out_roads[idx-1]["lanes"].keys())[0]
                        ))
                if direction_lst == [0, 1, 0]:
                    lane_link.append((
                        lane, list(self.out_roads[idx]["lanes"].keys())[1]
                        ))
                if direction_lst == [0, 0, 1]:
                    lane_link.append((
                        lane, list(self.out_roads[(idx+1)%len(self.out_roads)]["lanes"].keys())[2]
                        ))
        return lane_link
        
    def _get_in_out_lanes(self):
        in_lanes = []
        out_lanes = [lane for road in self.out_roads for lane in list(road["lanes"].keys())]
        for road in self.in_roads:
            from_zero = (road["start_inter"] == self.id) if self.RIGHT else (road["start_inter"] == self.id)
            in_lanes.append([int(road["id"])*100 + int(i) for i in range(road["num_lanes"])[::(1 if from_zero else -1)]])
        in_lanes = [item for sublist in in_lanes for item in sublist]
        return in_lanes, out_lanes
        
        
    def set_lane_link(self):
        self.lane_link = self._get_lane_link() 
        self.in_lanes, self.out_lanes = self._get_in_out_lanes()
        self.num_in_lanes = len(self.in_lanes)
        
        self.lane_link_idx = []
        for link in self.lane_link:
            from_lane = link[0]
            to_lane = link[1]
            self.lane_link_idx.append((self.in_lanes.index(from_lane), self.out_lanes.index(to_lane)))
        self.num_lane_link = len(self.lane_link)
...
@Registry.register_world('cbengine')
class World(object):
    """
    ...
    def __init__(self, cbengine_cfg, thread_num, **kwargs):
        print("building world...")
        with open(cbengine_cfg) as f:
            cbengine_cfg = json.load(f)
        engine_cfg = cbengine_cfg['engine_config']
        world_config = cbengine_cfg['world_config']
    ...
    def _generate_inter_road_map(self):
        data = []
        for inter_id, inter in self.all_intersections.items():
            roads = inter['start_roads'] + inter['end_roads']
            data.append({
                "inter_id": inter_id,
                "latitude": inter['latitude'],
                "longitude": inter['longitude'],
                "have_signal": inter['have_signal'],
                "roads": roads
            })
        df = pd.DataFrame(data)
        # df.to_csv("inter_road_map.csv", index=False)
    ...
    def get_vehicle_waiting_time(self):
        ...
        vehicles = self.eng.get_vehicles() # TODO CBEngine
    ...
    def get_pressure(self):
        ...
        pressures = {}
        for inter in self.intersections:
            in_lanes = inter.in_lanes
            out_lanes = inter.out_lanes
            pressure = 0
            for lane_id in in_lanes:
                pressure += lvc.get(lane_id, 0)
            for lane_id in out_lanes:
                pressure -= lvc.get(lane_id, 0)
            # if not inter.intersection.get("lanes"):
            #     continue
            # for lane in in_lanes:
            #     lane_id = inter.intersection["lanes"][lane-1]
            #     if lane_id in lvc:
    ...
    def step(self, actions=None):
        '''
            actions: a list, corresponding to the train_id
        '''
