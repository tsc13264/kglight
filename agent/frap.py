...
class FRAP_DQNAgent(RLAgent):
    ...
    def __init__(self, world, rank):
        ...
        # set valid action
        num_in_lanes = self.inter_obj.num_in_lanes
        if num_in_lanes == 12:
            self.phase_pairs = self.dic_traffic_env_conf.param['signal_config'][map_name]['phase_pairs']
        elif num_in_lanes == 9: 
            self.phase_pairs = self.dic_traffic_env_conf.param['signal_config'][map_name]['valid_acts']['three_way']
        elif num_in_lanes == 6:
            self.phase_pairs = self.dic_traffic_env_conf.param['signal_config'][map_name]['valid_acts']['two_way']
        else:
            raise ValueError("Invalid number of in-lanes")
    ...
    def _build_model(self):
        ...
        '''
        model = FRAP(self.dic_agent_conf, self.num_actions, self.phase_pairs, self.comp_mask).to(self.device)
        return model
    ...
    def get_ob(self):
        ...
        # tmp = np.clip(tmp, a_min=None, a_max=10)  
        x_obs.append(tmp)
        return x_obs
    ...
    def load_model(self, e):
        ...
        self.model = FRAP(self.dic_agent_conf, self.dic_phase_expansion, self.num_actions, self.phase_pairs, self.comp_mask).to(self.device)
        self.model.load_state_dict(torch.load(model_name))
        self.target_model = FRAP(self.dic_agent_conf, self.dic_phase_expansion, self.num_actions, self.phase_pairs, self.comp_mask).to(self.device)
