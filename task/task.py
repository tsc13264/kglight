...
class TSCTask(BaseTask):
    ...
    def __init__(self, trainer):
        ...
        self.load_model = Registry.mapping['trainer_mapping']['setting'].param['load_model']
    ...
    def run(self):
        ...
        try:
            if Registry.mapping['model_mapping']['setting'].param['train_model']:
                self.trainer.train()
            if Registry.mapping['command_mapping']['setting'].param['agent'] == 'pilight':
                self.trainer.synthesize()
            if Registry.mapping['model_mapping']['setting'].param['test_model']:
                self.trainer.test(self.load_model)
