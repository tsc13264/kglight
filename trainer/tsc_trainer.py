...
class TSCTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.test_when_train_start = Registry.mapping['trainer_mapping']['setting'].param['test_when_train_start']
        ...
        self.writer = SummaryWriter(log_dir=os.path.join(
            Registry.mapping['logger_mapping']['path'].path, "writer", 
            os.path.basename(self.logger.handlers[-1].baseFilename).rstrip('_BRF.log'))
            )
        ...
        self.writer_prefix = f"{map_name}_{agent_name}"
        self.action_writer = False and Registry.mapping['command_mapping']['setting'].param['agent'] == "ecolight"
        self.att_writer = True and Registry.mapping['command_mapping']['setting'].param['prefix'] != "test"
        
        self.save_replay = Registry.mapping['world_mapping']['setting'].param['saveReplay']
        self.LOG_ADDR = Registry.mapping['world_mapping']['setting'].param['report_log_addr']
        ...
        for e in tqdm(range(self.episodes)):
            ...
            while i < self.steps:
                if i % self.action_interval == 0:
                    ...
                    if self.action_writer:
                        self.writer.add_scalar(f"{self.writer_prefix}_action_writer", actions[0], global_step=total_decision_num)
            ...
            if self.att_writer:
                self.writer.add_scalar(f"{self.writer_prefix}_average_travel_time", self.metric.real_average_travel_time(), global_step=e)
            
            self.writeLog("TRAIN", e, self.metric.real_average_travel_time(),\
                mean_loss, self.metric.rewards(), self.metric.queue(), delay, self.metric.throughput())
            
            self.logger.info("Episode:{}/{}, Train step:{}/{}, rewards:{:.2f}, queue:{:.2f}, delay:{:.2f}, throughput:{}, avg travel time:{:.2f}".format(
                e, self.episodes, i, self.steps, self.metric.rewards(), self.metric.queue(), delay, int(self.metric.throughput()), self.metric.real_average_travel_time()
                ))
            ...
            if self.test_when_train and ((e >= self.test_when_train_start and e % self.test_when_train_rate == 0) or (e >= self.episodes - 10)):
                self.train_test(e)
        # self.dataset.flush([ag.replay_buffer for ag in self.agents])
    ...
    def train_test(self, e):
        ...
        for i in range(self.test_steps):
            ...
            if all(dones):
                break
        self.logger.info("Episode:{}/{}, Test  step:{}/{}, rewards:{:.2f}, queue:{:.2f}, delay:{:.2f}, throughput:{}, avg travel time:{:.2f}".format(
                e, self.episodes, i, self.steps, self.metric.rewards(), self.metric.queue(), delay, int(self.metric.throughput()), self.metric.real_average_travel_time()
                ))
        self.writeLog("TEST", e, self.metric.real_average_travel_time(),\
            100, self.metric.rewards(),self.metric.queue(),delay, self.metric.throughput())
    ...
    def test(self, load_model=False):
        ...
        :param load_model: decide whether to load pretrained model's parameters
        ...
        for i in range(self.test_steps):
            if i % self.action_interval == 0:
                ...
                for j in range(self.action_interval):
                    self.world.eng.log_info(os.path.join(self.LOG_ADDR, 'time{}.json'.format(int(self.world.eng.get_current_time())))) if self.save_replay else None
                    obs, rewards, dones, _ = self.env.step(actions.flatten())
                    i += 1
                    rewards_list.append(np.stack(rewards))
                rewards = np.mean(rewards_list, axis=0)  # [agent, intersection]
                ...
                for j in range(len(self.world.intersections)):
                    self.logger.debug("Train step:{}/{}, intersection:{}, obs:{}".format(i, self.test_steps, j, obs[j].sum()))
                    
            if all(dones):
                break
            
        self.logger.info("Final Test: mean rewards:{:.2f}, queue:{:.2f}, delay:{:.2f}, throughput:{}, avg travel time:{:.2f}".format(
                self.metric.rewards(), self.metric.queue(), delay, int(self.metric.throughput()), self.metric.real_average_travel_time()
                ))
    ...
    def synthesize(self):
        from agent.pilight.MCTS import MCTS_synthesizer
        synthesizer = MCTS_synthesizer(self.env)
        ...
        print(agent)
        num_agent = int(len(self.world.intersections) / agent.sub_agents)
        self.target_agents.append(agent)  # initialized N agents for traffic light control
        for i in range(1, num_agent):
            self.target_agents.append(Registry.mapping['model_mapping'][Registry.mapping['command_mapping']['setting'].param['agent']](self.world, i))
        
        lane_metrics = ['rewards', 'queue']
        world_metrics = ['real avg travel time', 'throughput']
        self.metric = Metrics(lane_metrics, world_metrics, self.world, self.target_agents)
        
        for i, a in enumerate(self.agents):
            a.reset()
            self.target_agents[i].inject_code(a.inlane_code, a.outlane_code)
        
        for i in range(self.test_steps):
            if i % self.action_interval == 0:
                phases = np.stack([ag.get_phase() for ag in self.target_agents])
                actions = []
                for idx, ag in enumerate(self.target_agents):
                    actions.append(ag.get_action(obs, None))
                actions = np.stack(actions)
                rewards_list = []
                for j in range(self.action_interval):
                    self.world.eng.log_info(os.path.join(self.LOG_ADDR, 'time{}.json'.format(int(self.world.eng.get_current_time())))) if self.save_replay else None
