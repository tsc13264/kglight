...
def modify_config_file(path, config):
    ...
    #                 contents[idx] = lhs + '=' + rhs
    #             if lhs.strip() == 'interval':
    #                 rhs = ' ' + str(param['report_log_rate']) + '\n'
    #         elif ':' in l:
    #             lhs, _ = l.split(':')
    ...
    elif config['command']['world'] == 'cbengine':
        with open(path, 'r') as f:
            path_config = json.load(f)
        with open(path_config['engine_config'], 'r') as f:
            contents = f.readlines()
        for idx, l in enumerate(contents):
            if '=' in l:
                lhs, _ = l.split('=')
                ...
                if lhs.strip() == 'report_log_addr':
                    ...
                    rhs = ' ' + file_name + '\n'
                    contents[idx] = lhs + ':' + rhs
                    param['report_log_addr'] = file_name
                    
        with open(path_config['engine_config'], 'w') as f:
            f.writelines(contents)
